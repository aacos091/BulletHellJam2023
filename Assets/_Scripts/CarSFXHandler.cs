...
public class CarSFXHandler : MonoBehaviour
{
    ...
    public AudioSource tiresScreechAudioSource;
    ...
    {
        // Handle engine SFX
        float velocityMagnitude = _vehicle.GetVelocityMagnitude();
    ...
    void UpdateTiresScreechSFX()
    {
        ...
        if (_vehicle.IsTireScreeching(out float lateralVelocity, out bool isBraking))
        {
            ...
            if (isBraking)
            {
                tiresScreechAudioSource.volume = Mathf.Lerp(tiresScreechAudioSource.volume, 1.0f, Time.deltaTime * 10);
                tireScreechPitch = Mathf.Lerp(tireScreechPitch, 0.5f, Time.deltaTime * 10);
            }
            else
            {
                //If we are not braking we still want to play this screech sound if the player is drifting.
                tiresScreechAudioSource.volume = Mathf.Abs(lateralVelocity) * 0.05f;
                tireScreechPitch = Mathf.Abs(lateralVelocity) * 0.1f;
            }
        }
        //Fade out the tire screech SFX if we are not screeching. 
        else tiresScreechAudioSource.volume = Mathf.Lerp(tiresScreechAudioSource.volume, 0, Time.deltaTime * 10);
    }
